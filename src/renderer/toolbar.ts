...
export class Toolbar {
  ...
  constructor(host: HTMLElement, h: ToolbarHandlers) {
    ...
    const HL_COLOURS: HighlightColour[] = ['yellow', 'green', 'blue', 'pink']
    ...
    for (const c of HL_COLOURS) {
      const sw = document.createElement('button')
      sw.className = 'tb-swatch hl-' + c; sw.title = c
  ...
  syncHighlighter(on: boolean, colour: HighlightColour): void {
    ...
    this.hlBtn.style.color = on ? `var(--accent-text)` : ''
    this.hlBtn.dataset.colour = colour
  }
}
