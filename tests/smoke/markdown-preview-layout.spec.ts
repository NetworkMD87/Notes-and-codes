...
test('an untitled buffer converts for preview, persists Markdown mode, and follows Save As extensions', async ({ smoke }) => {
  ...
  const chooser = win.locator('[data-toolbar="markdown-preview-mode"]')
  ...
  await win.keyboard.insertText('abc')
  await win.keyboard.press('Control+Home')
  await win.keyboard.press('ArrowRight')
  await win.keyboard.press('Shift+ArrowRight')
  await chooseToolbarMode(win, 'Side by side')
  await expectMode(win, 'side-by-side')
  await expect(win.locator('#mdpreview')).toHaveText('abc')
  await expect(win.locator('#statusbar')).toContainText('Ln 1, Col 3')
  const editorInput = win.locator('#paneA textarea.inputarea')
  await editorInput.focus()
  await editorInput.press('X')
  await expect(win.locator('#paneA .view-lines')).toContainText('aXc')
  await win.keyboard.press('Control+z')
  await expect(win.locator('#paneA .view-lines')).toContainText('abc')
  await win.keyboard.press('Control+z')
  await expect(win.locator('#paneA .view-lines')).toHaveText('')
